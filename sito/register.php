...
if (isset($_POST['submit-sign-up'])) {
    ...
    if (mb_strlen($email)==0) {
        ...
        $error=true;
        $pagina = str_replace("[email-error]",'<p role="alert" class="error" id="email-error">L\'<span lang="en">email</span> non è un indirizzo valido</p>',$pagina);
    } else {
        ...
        if (strcmp($isEmailExist,"ExceptionThrow")!=0 && strcmp($isEmailExist,"Connection error")!=0 && $isEmailExist==true) {
            $error=true;
            $pagina = str_replace("[email-error]",'<p role="alert" class="error" id="email-error">Questa <span lang="en">email</span> non può essere utilizzata</p>',$pagina);
    ...
    if ($error==false) {
        ...
        $dataString = $date->format('Y-m-d');
        $pagina = str_replace('id="register_datanascita"','id="register_datanascita" value="'.$dataString.'"',$pagina);

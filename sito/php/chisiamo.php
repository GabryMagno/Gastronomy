...
if(!isset($_POST["advice"])) {
    ...
    if(isset($_SESSION["adviceError"])) {//se c'è un errore nella sessione, lo mostra
        $pagina=str_replace("{{advice-error}}",$_SESSION["adviceError"],$pagina);//se ci sono errori, mostra il messaggio di errore
        ...
        $pagina=str_replace("{{suggestion-error}}","",$pagina);//se non ci sono errori, rimuove il messaggio di errore
    }
    echo $pagina;
} else {
    ...
    if($result==false) {//se c'è un errore nell'aggiungere il suggerimento, reindirizza alla pagina di errore
        ...
        header('Location: ringraziamento.php');//faremo una pagina di ringraziamento
        exit();
    }
}
